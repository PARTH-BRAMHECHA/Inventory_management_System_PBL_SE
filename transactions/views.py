...
class SalesBarGraphView(TemplateView):
    template_name = "sales/sales_bar_graph.html"

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        
        # Calculate sales data for each customer
        sales_data = self.calculate_sales_data()

        # Pass sales data to the template
        context['sales_data'] = sales_data
        return context

    def calculate_sales_data(self):
        # Query sales data aggregated by customer
        sales_by_customer = SaleBill.objects.values('name').annotate(total_sales=Count('billno'))

        # Extract customer names and total sales from the queryset
        labels = [sale['name'] for sale in sales_by_customer]
        values = [sale['total_sales'] for sale in sales_by_customer]
